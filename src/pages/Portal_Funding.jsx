    ...
    return (
      <div className="p-6 max-w-2xl mx-auto">
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-6 text-center">
          ...
          <p className="text-sm text-[#8b8fa8]">Los fondos solo están disponibles para cuentas Reales. Cambia a cuenta REAL en el encabezado para acceder a depósitos y retiros.</p>
        </div>
      </div>
    );
  }
      ...
      {!kycApproved && (
        <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4 flex items-start gap-3">
          ...
          <div className="text-sm">
            <strong className="text-yellow-400">Verificación requerida para retiros</strong>
            <p className="text-[#8b8fa8] mt-0.5">Completa la verificación KYC para habilitar retiros. Los depósitos están disponibles de inmediato.</p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Tabs value={tab} onValueChange={setTab}>
            ...
            {['deposit', 'withdraw'].map(type => (
              <TabsContent key={type} value={type} className="m-0 p-5 space-y-4">
                <div>
                  <Label className="text-xs text-[#8b8fa8] uppercase">Monto (USD)</Label>
                  <input
                    type="number"
                    inputMode="decimal"
                    min="0"
                    ...
                    className="mt-1.5 w-full rounded-md border border-[#1e2130] bg-[#131722] px-3 py-3 text-white font-mono text-lg focus:outline-none focus:ring-2 focus:ring-[#2196F3] placeholder:text-[#4a5568]"
                  />
                  {/* Quick amount buttons */}
                  <div className="flex gap-2 mt-2">
                    {[100, 500, 1000, 5000].map(v => (
                      <button key={v} type="button" onClick={() => setAmount(String(v))}
                        className="flex-1 py-1.5 text-[11px] font-semibold rounded-md bg-[#1e2130] text-[#8b8fa8] hover:bg-[#252836] hover:text-white transition-colors">
                        ${v}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  <Label className="text-xs text-[#8b8fa8] uppercase">Método de pago</Label>
                  <Select value={method} onValueChange={setMethod}>
                    <SelectTrigger className="mt-1.5 bg-[#131722] border-[#1e2130] text-white">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent className="bg-[#1e2130] border-[#2a2e3f]">
                      <SelectItem value="bank_transfer" className="text-[#d1d4dc] text-xs">Transferencia bancaria</SelectItem>
                      <SelectItem value="credit_card" className="text-[#d1d4dc] text-xs">Tarjeta de crédito</SelectItem>
                      <SelectItem value="debit_card" className="text-[#d1d4dc] text-xs">Tarjeta de débito</SelectItem>
                      <SelectItem value="wire_transfer" className="text-[#d1d4dc] text-xs">Wire transfer</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div>
                  <Label className="text-xs text-[#8b8fa8] uppercase">Referencia / Notas</Label>
                  <input
                    value={notes}
                    onChange={e => setNotes(e.target.value)}
                    placeholder="Referencia opcional..."
                    style={{ colorScheme: 'dark' }}
                    className="mt-1.5 w-full rounded-md border border-[#1e2130] bg-[#131722] px-3 py-2.5 text-white text-xs focus:outline-none focus:ring-2 focus:ring-[#2196F3] placeholder:text-[#4a5568]"
                  />
                </div>
                {type === 'withdraw' && !kycApproved && (
                  <div className="text-xs text-[#ef5350] bg-[#ef5350]/10 rounded-lg p-3">
                    ⚠ Completa la verificación KYC antes de retirar fondos.
                  </div>
                )}
                <Button onClick={() => handleSubmit(type)} disabled={submitting || (type === 'withdraw' && !kycApproved)}
                  className={`w-full font-semibold h-12 text-base ${type === 'deposit' ? 'bg-[#26a69a] hover:bg-[#26a69a]/90' : 'bg-[#ef5350] hover:bg-[#ef5350]/90'}`}>
                  {submitting ? 'Procesando…' : type === 'deposit' ? 'Solicitar depósito' : 'Solicitar retiro'}
                </Button>
              </TabsContent>
            ))}
          </Tabs>
        ...
        <div className="space-y-3">
          <div className="bg-[#0f1117] border border-[#1e2130] rounded-xl p-5">
            <h3 className="text-xs text-[#8b8fa8] uppercase mb-3">Saldo de la cuenta</h3>
            <div className="text-3xl font-bold font-mono text-white mb-1">
              ${(activeAccount?.cash_balance || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}
            </div>
            <div className="text-xs text-[#8b8fa8]">Efectivo disponible — USD</div>
          </div>
          <div className="bg-[#0f1117] border border-[#1e2130] rounded-xl p-4 text-xs text-[#8b8fa8] space-y-1">
            <div className="flex justify-between"><span>Total depositado:</span><span className="text-white font-mono">${ledger.filter(l => l.type === 'deposit' && l.status === 'completed').reduce((s, l) => s + l.amount, 0).toFixed(2)}</span></div>
            <div className="flex justify-between"><span>Total retirado:</span><span className="text-white font-mono">${ledger.filter(l => l.type === 'withdrawal' && l.status === 'completed').reduce((s, l) => s + l.amount, 0).toFixed(2)}</span></div>
            <div className="flex justify-between"><span>Pendientes:</span><span className="text-yellow-400 font-mono">{ledger.filter(l => l.status === 'pending').length} solicitud(es)</span></div>
          </div>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {sorted.length === 0 && <tr><td colSpan={6} className="p-8 text-center text-[#8b8fa8] text-sm">Sin transacciones aún</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
